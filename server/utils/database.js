...
const validateForm = (username, password ) => {
  ...
  if (!VALIDATION_REG['username'].test(username) || !VALIDATION_REG['password'].test(password)) {
    return false;
  }
  return true;
}
...
const saveUserData = (userData) => {
  return new Promise((resolve, reject) => {
    ...
    fs.writeFile(getDataPathByUser(username), JSON.stringify(user), 'utf-8', (err) => {
      if (err) {
        const error = new Error('Fail to save user.');
        return reject(error);
